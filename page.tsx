...
const MAIN_CDN = "https://kamulogstk.net";
const resolveUrl = (url?: string | null): string => {
  if (!url) return "";
  if (url.startsWith("http")) return url;
  if (url.startsWith("data:")) return url;
  if (url.startsWith("/")) return MAIN_CDN + url;
  return MAIN_CDN + "/" + url;
};
...
export default function ProfilPage() {
    ...
    try { const r=await fetch("/api/stk-panel/profile"); const j=await r.json();
      if(j.success&&j.data){ setP(j.data);
        setForm({email:j.data.email||"",phone:j.data.phone||"",website:j.data.website||"",city:j.data.city||"",district:j.data.district||"",address:j.data.address||"",description:j.data.description||"",
          ...
          consentText:j.data.consentText||"",contractPdfUrl:j.data.contractPdfUrl||"",logo:j.data.logo||"",branchesPublic:j.data.branchesPublic!==false,activitiesPublic:j.data.activitiesPublic===true,decisionsPublic:j.data.decisionsPublic===true,chatPublic:j.data.chatPublic===true,chatDisabledForMembers:j.data.chatDisabledForMembers===true,
          acceptsDonation:j.data.acceptsDonation||false,acceptsDues:j.data.acceptsDues||false,acceptsAnnualDues:j.data.acceptsAnnualDues||false,requiresMembershipForFinance:j.data.requiresMembershipForFinance??true,showMemberCount:j.data.showMemberCount||false,showFacebookOnMobile:j.data.showFacebookOnMobile!==false,showInstagramOnMobile:j.data.showInstagramOnMobile!==false,showTwitterOnMobile:j.data.showTwitterOnMobile!==false,showYoutubeOnMobile:j.data.showYoutubeOnMobile!==false,showTelegramOnMobile:j.data.showTelegramOnMobile!==false,showWhatsappOnMobile:j.data.showWhatsappOnMobile!==false,showWebsiteOnMobile:j.data.showWebsiteOnMobile!==false,isConsentActive:j.data.isConsentActive||false,isApplicationEnabled:j.data.isApplicationEnabled??true,
          boardPublic:j.data.boardPublic!==false,
          registrationNo:j.data.registrationNo||"",facebookUrl:j.data.facebookUrl||"",twitterUrl:j.data.twitterUrl||"",instagramUrl:j.data.instagramUrl||"",youtubeUrl:j.data.youtubeUrl||"",whatsappGroupUrl:j.data.whatsappGroupUrl||"",telegramUrl:j.data.telegramUrl||"",whatsappLinks:j.data.whatsappLinks||[],
        });
      }
  ...
  const tabs = [{k:"info",l:"📝 İletişim"},{k:"corp",l:"🏢 Kurumsal"},{k:"finance",l:"💰 Finans"},{k:"settings",l:"⚙️ Ayarlar"},{k:"social",l:"🌐 Sosyal Medya"},{k:"docs",l:"📂 Belgeler"}];
    ...
    <div className="space-y-6" style={{animation:"fadeIn .5s ease-out"}}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-4">
          <div className="relative group">
            {form.logo ? <img src={resolveUrl(form.logo)} alt="Logo" className="w-16 h-16 rounded-2xl object-cover border-2 border-indigo-100"/> : <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-indigo-100 to-violet-100 flex items-center justify-center text-2xl font-bold text-indigo-700">{p?.name?.charAt(0)||"S"}</div>}
      ...
      {tab==="settings"&&(
        <div className="space-y-6">
          ...
          <div className="rounded-2xl bg-white border border-gray-100 shadow-sm p-6">
            <h2 className="font-semibold text-gray-900 mb-2">📱 Mobil Buton Görünürlük Ayarları</h2>
            <p className="text-[11px] text-gray-400 mb-4">Her butonu ayrı ayrı &quot;Herkese Açık&quot; veya &quot;Sadece Üyelere Özel&quot; yapabilirsiniz.</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {[
                {key:"boardPublic",label:"💼 Yönetim Kurulu",desc:"Yönetim kurulu üyeleri"},
                {key:"branchesPublic",label:"🏢 Teşkilatımız",desc:"Şube ve temsilci bilgileri"},
                {key:"activitiesPublic",label:"📋 Dernek Faaliyetleri",desc:"Etkinlik ve faaliyet duyuruları"},
                {key:"decisionsPublic",label:"📜 Yönetim Kurulu Kararları",desc:"YK karar metinleri"},
                {key:"chatPublic",label:"💬 Üye Sohbet Odası",desc:"Üyeler arası mesajlaşma"},
              ].map(item=>(
                <label key={item.key} className={`flex items-start gap-3 p-3 rounded-xl border cursor-pointer transition-all ${form[item.key]?"bg-emerald-50 border-emerald-200":"bg-gray-50 border-gray-200"}`}>
                  <input type="checkbox" checked={form[item.key]||false} onChange={e=>setForm((f:any)=>({...f,[item.key]:e.target.checked}))} className="mt-0.5 w-4 h-4 rounded text-emerald-600"/>
                  <div>
                    <p className="text-xs font-semibold text-gray-800">{item.label}</p>
                    <p className="text-[10px] text-gray-500">{item.desc}</p>
                    <span className={`inline-block mt-1 px-2 py-0.5 rounded text-[9px] font-bold ${form[item.key]?"bg-emerald-100 text-emerald-700":"bg-amber-100 text-amber-700"}`}>{form[item.key]?"🌍 Herkese Açık":"🔒 Sadece Üyelere Özel"}</span>
                  </div>
                </label>
              ))}
            </div>
          ...
          <div className="rounded-2xl bg-white border border-gray-100 shadow-sm p-6">
            <h2 className="font-semibold text-gray-900 mb-2">🚫 Sohbet Odası Yönetimi</h2>
            <p className="text-[11px] text-gray-400 mb-4">Üyelere bile sohbet odasını kapatabilirsiniz.</p>
            <label className={`flex items-start gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all ${form.chatDisabledForMembers?"border-red-300 bg-red-50":"border-gray-200 bg-gray-50"}`}>
              <input type="checkbox" checked={form.chatDisabledForMembers||false} onChange={e=>setForm((f:any)=>({...f,chatDisabledForMembers:e.target.checked}))} className="mt-0.5 w-5 h-5 rounded text-red-600"/>
              <div>
                <p className="text-sm font-semibold text-gray-800">🛑 Sohbet Odasını Tamamen Kapat</p>
                <p className="text-xs text-gray-500 mt-1">Üyeler dahil hiç kimse mesaj gönderemez. Bakım veya moderasyon için kullanın.</p>
                <span className={`inline-block mt-2 px-3 py-1 rounded-lg text-xs font-bold ${form.chatDisabledForMembers?"bg-red-100 text-red-700":"bg-emerald-100 text-emerald-700"}`}>{form.chatDisabledForMembers?"🔴 Sohbet KAPALI":"🟢 Sohbet AÇIK"}</span>
              </div>
            </label>
          </div>
          <div className="rounded-2xl bg-white border border-gray-100 shadow-sm p-6">
      ...
      {tab==="social"&&(
        <div className="space-y-5">
          <div className="p-5 rounded-2xl" style={{background:"rgba(255,255,255,0.8)",border:"1px solid rgba(0,0,0,0.06)"}}>
            <h3 className="text-sm font-bold text-slate-800 mb-4 flex items-center gap-2">🏛️ Kurum Sicil Bilgileri</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div><label className="block text-[10px] font-bold text-slate-500 mb-1 uppercase">Sicil / Kütük No</label><input value={form.registrationNo||""} onChange={e=>setForm({...form,registrationNo:e.target.value})} className="w-full px-4 py-3 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20" style={{background:"#f8fafc",border:"1.5px solid #e2e8f0"}} placeholder="06-160-048"/></div>
            </div>
          </div>
          <div className="p-5 rounded-2xl" style={{background:"rgba(255,255,255,0.8)",border:"1px solid rgba(0,0,0,0.06)"}}>
            <h3 className="text-sm font-bold text-slate-800 mb-4 flex items-center gap-2">🌐 Sosyal Medya Hesapları</h3>
            <p className="text-xs text-slate-400 mb-4">Linkleri kopyalayıp yapıştırın. Bu bilgiler ana sayfada vatandaşlara gösterilecektir.</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="flex flex-col gap-1 p-3 border rounded-xl bg-[#f8fafc]">
                <div className="flex justify-between items-center mb-1">
                  <label className="text-[10px] font-bold text-slate-500 uppercase">📘 Facebook</label>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <span className="text-[10px] font-semibold text-slate-500">Mobilde Göster</span>
                    <input type="checkbox" checked={form.showFacebookOnMobile!==false} onChange={e=>setForm({...form,showFacebookOnMobile:e.target.checked})} className="w-3 h-3 text-emerald-600 rounded"/>
                  </label>
                </div>
                <input value={form.facebookUrl||""} onChange={e=>setForm({...form,facebookUrl:e.target.value})} className="w-full px-3 py-2.5 rounded-lg text-sm bg-white border focus:outline-none focus:ring-2 focus:ring-emerald-500/20 border-slate-200" placeholder="https://facebook.com/..."/>
              </div>
              <div className="flex flex-col gap-1 p-3 border rounded-xl bg-[#f8fafc]">
                <div className="flex justify-between items-center mb-1">
                  <label className="text-[10px] font-bold text-slate-500 uppercase">𝕏 Twitter / X</label>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <span className="text-[10px] font-semibold text-slate-500">Mobilde Göster</span>
                    <input type="checkbox" checked={form.showTwitterOnMobile!==false} onChange={e=>setForm({...form,showTwitterOnMobile:e.target.checked})} className="w-3 h-3 text-emerald-600 rounded"/>
                  </label>
                </div>
                <input value={form.twitterUrl||""} onChange={e=>setForm({...form,twitterUrl:e.target.value})} className="w-full px-3 py-2.5 rounded-lg text-sm bg-white border focus:outline-none focus:ring-2 focus:ring-emerald-500/20 border-slate-200" placeholder="https://x.com/..."/>
              </div>
              <div className="flex flex-col gap-1 p-3 border rounded-xl bg-[#f8fafc]">
                <div className="flex justify-between items-center mb-1">
                  ...
                  <label className="flex items-center gap-2 cursor-pointer">
                    <span className="text-[10px] font-semibold text-slate-500">Mobilde Göster</span>
                    <input type="checkbox" checked={form.showInstagramOnMobile!==false} onChange={e=>setForm({...form,showInstagramOnMobile:e.target.checked})} className="w-3 h-3 text-emerald-600 rounded"/>
                  </label>
                </div>
                <input value={form.instagramUrl||""} onChange={e=>setForm({...form,instagramUrl:e.target.value})} className="w-full px-3 py-2.5 rounded-lg text-sm bg-white border focus:outline-none focus:ring-2 focus:ring-emerald-500/20 border-slate-200" placeholder="https://instagram.com/..."/>
              </div>
              <div className="flex flex-col gap-1 p-3 border rounded-xl bg-[#f8fafc]">
                <div className="flex justify-between items-center mb-1">
                  <label className="text-[10px] font-bold text-slate-500 uppercase">▶️ YouTube</label>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <span className="text-[10px] font-semibold text-slate-500">Mobilde Göster</span>
                    <input type="checkbox" checked={form.showYoutubeOnMobile!==false} onChange={e=>setForm({...form,showYoutubeOnMobile:e.target.checked})} className="w-3 h-3 text-emerald-600 rounded"/>
                  </label>
                </div>
                <input value={form.youtubeUrl||""} onChange={e=>setForm({...form,youtubeUrl:e.target.value})} className="w-full px-3 py-2.5 rounded-lg text-sm bg-white border focus:outline-none focus:ring-2 focus:ring-emerald-500/20 border-slate-200" placeholder="https://youtube.com/..."/>
              </div>
              <div className="flex flex-col gap-1 p-3 border rounded-xl bg-[#f8fafc]">
                <div className="flex justify-between items-center mb-1">
                  <label className="text-[10px] font-bold text-slate-500 uppercase">✈️ Telegram</label>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <span className="text-[10px] font-semibold text-slate-500">Mobilde Göster</span>
                    <input type="checkbox" checked={form.showTelegramOnMobile!==false} onChange={e=>setForm({...form,showTelegramOnMobile:e.target.checked})} className="w-3 h-3 text-emerald-600 rounded"/>
                  </label>
                </div>
                <input value={form.telegramUrl||""} onChange={e=>setForm({...form,telegramUrl:e.target.value})} className="w-full px-3 py-2.5 rounded-lg text-sm bg-white border focus:outline-none focus:ring-2 focus:ring-emerald-500/20 border-slate-200" placeholder="https://t.me/..."/>
              </div>
              <div className="flex flex-col gap-1 p-3 border rounded-xl bg-[#f8fafc] sm:col-span-2">
                <div className="flex justify-between items-center mb-1">
                  <label className="text-[10px] font-bold text-slate-500 uppercase">💬 WhatsApp Grup/Kanal (Tek Link)</label>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <span className="text-[10px] font-semibold text-slate-500">Mobilde Göster</span>
                    <input type="checkbox" checked={form.showWhatsappOnMobile!==false} onChange={e=>setForm({...form,showWhatsappOnMobile:e.target.checked})} className="w-3 h-3 text-emerald-600 rounded"/>
                  </label>
                </div>
                <input value={form.whatsappGroupUrl||""} onChange={e=>setForm({...form,whatsappGroupUrl:e.target.value})} className="w-full px-3 py-2.5 rounded-lg text-sm bg-white border focus:outline-none focus:ring-2 focus:ring-emerald-500/20 border-slate-200" placeholder="https://chat.whatsapp.com/..."/>
              </div>
              <div className="sm:col-span-2 mt-4">
                <div className="flex items-center justify-between mb-2"><label className="block text-[10px] font-bold text-emerald-600 uppercase">💬 Çoklu WhatsApp Grupları/Kanalları</label><button type="button" onClick={()=>setForm({...form,whatsappLinks:[...(form.whatsappLinks||[]),{name:"",url:""}]})} className="px-3 py-1.5 rounded-lg bg-emerald-50 text-emerald-700 text-xs font-bold hover:bg-emerald-100 transition">+ Yeni Ekle</button></div>
                <p className="text-[10px] text-slate-400 mb-3">Birden fazla WhatsApp grubu veya kanalı ekleyebilirsiniz. Her birine isim verin.</p>
                {(form.whatsappLinks||[]).map((link: any, i: number)=>(
                  <div key={i} className="flex items-center gap-2 mb-2">
                    <input value={link.name||""} onChange={e=>{const links=[...(form.whatsappLinks||[])];links[i]={...links[i],name:e.target.value};setForm({...form,whatsappLinks:links})}} className="w-1/3 px-3 py-2.5 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20" style={{background:"#f8fafc",border:"1.5px solid #e2e8f0"}} placeholder="Grup Adı"/>
                    <input value={link.url||""} onChange={e=>{const links=[...(form.whatsappLinks||[])];links[i]={...links[i],url:e.target.value};setForm({...form,whatsappLinks:links})}} className="flex-1 px-3 py-2.5 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20" style={{background:"#f8fafc",border:"1.5px solid #e2e8f0"}} placeholder="https://chat.whatsapp.com/..."/>
                    <button type="button" onClick={()=>{const links=[...(form.whatsappLinks||[])];links.splice(i,1);setForm({...form,whatsappLinks:links})}} className="w-8 h-8 rounded-lg bg-red-50 text-red-500 text-xs font-bold hover:bg-red-100 flex items-center justify-center">✕</button>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="mb-3">
                <div className="flex items-center gap-3 rounded-xl bg-gradient-to-r from-indigo-50 to-violet-50 border border-indigo-100 p-3 hover:shadow-md transition-all">
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <a href={form.contractPdfUrl.startsWith("http") ? form.contractPdfUrl : `/api/download?file=${encodeURIComponent(form.contractPdfUrl)}`} target="_blank" rel="noreferrer" className="text-indigo-600 hover:text-indigo-800 text-xs font-medium">📥 İndir</a>
            ...
            {p?.STKDocument&&p.STKDocument.length>0 ? (
              <div className="space-y-3">{p.STKDocument.map((d:any)=>(
                <div key={d.id} className="flex items-center gap-3 rounded-xl bg-gray-50 border border-gray-100 p-3 hover:bg-indigo-50/50 transition-all">
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <a href={d.fileUrl.startsWith("http") ? d.fileUrl : `/api/download?file=${encodeURIComponent(d.fileUrl)}`} target="_blank" rel="noreferrer" className="text-indigo-600 hover:text-indigo-800 text-xs font-medium">📥 İndir</a>
