...
// Mobil uygulama için public API — aynı market-data ama auth yok
...
async function fetchMarketData() {
    ...
    try {
        ...
        if (btcRes.ok) {
            ...
            if (btcData.bitcoin) {
                results.btc = { rate: btcData.bitcoin.try, change24h: btcData.bitcoin.try_24h_change?.toFixed(2) || "0", symbol: "₿" };
            }
        }
    } catch (e) { console.error(e); }
    ...
    try {
        ...
        if (bistRes.ok) {
            ...
            if (meta) {
                ...
                results.bist100 = { rate: Math.round(current), change: prev ? ((current - prev) / prev * 100).toFixed(2) : "0", symbol: "📈" };
            }
        }
    } catch (e) { console.error(e); }
