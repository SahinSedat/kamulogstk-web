...
export default function YonetimKuruluPage() {
  ...
  const [form, setForm] = useState({ name: "", title: "Asil Üye", phone: "", email: "" });
  ...
  const openAdd = () => {
    setEditId(null);
    setForm({ name: "", title: "Asil Üye", phone: "", email: "" });
    setShowModal(true);
  };

  const openEdit = (m: BoardMember) => {
    setEditId(m.id);
    setForm({ name: m.name, title: m.title, phone: m.phone ? stripPhonePrefix(m.phone) : "", email: m.email || "" });
    setShowModal(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editId) {
        ...
        const json = await res.json();
        if (json.success) { showToastMsg("✅ Kurul üyesi eklendi!", "success"); setShowModal(false); setForm({ name: "", title: "Asil Üye", phone: "", email: "" }); fetchData(); }
        else showToastMsg(json.error || "Hata", "error");
      }
            ...
            return (
              <div key={m.id} className="rounded-2xl bg-white border border-gray-100 shadow-sm p-5 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group">
                <div className="flex items-start gap-4">
                  <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-indigo-100 to-violet-100 flex items-center justify-center text-lg font-bold text-indigo-700 flex-shrink-0">
